...
import CustomCalendar2 from './CustomCalendar';
...
export default function WriteDiary({ route }) {
    ...
    const [lock_State, setLock_State] = useState('unlock');
    // 요일 정보를 가져오기 위한 배열
    ...
    const writtenDiaryDates = ['2023-10-24'];

    // 위에서 가져온 날짜들을 markedDates 형태로 변환합니다.
    ...
    return (
        ...
        >
            <View style={styles.header}>
                ...
                <TouchableOpacity onPress={() => setLock_State(lock_State === 'unlock' ? 'lock' : 'unlock')}>
                    {lock_State === 'unlock' ? (
                        <Image
    ...
    function addDiary() {
        ...
        axios.post(`http://116.125.97.34:9090/getSentiment`, { content: content })
            .then(response => {
                ...
                return axios.post(address + '/saveDiary', {
                    name: sendName,
                    title: sendTitle,
                    year: sendYear,
                    month: sendMonth,
